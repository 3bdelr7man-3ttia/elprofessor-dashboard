...
(function () {
  ...
  // F-001: رقم الواتساب بيتبعت مع الدعوة — عليه بيتبعت الرابط، وبيه بيتفتح (المنصة بترفض بلا رقم).
  ...
  EP.createInvite = function (data, onLink, after) {
    post("/invites", { name: data.name || "", email: data.email || "", phone: data.phone || "", note: data.note || "" })
      .then(function (r) {
  ...
  // F-012 «أعِد إصدارها»: دعوة منتهية/مسحوبة/مرفوضة بتترجّع للحياة برمزٍ **جديد** ومهلة جديدة.
  // ⛔ الرابط القديم بيفضل ميّت وبيردّ «الدعوة انتهت» (٤١٠) — مش بيرجع يشتغل. اللي بيرجّع من
  //    المنصة هو الرابط الجديد، وهو اللي بيتبعت على واتساب. الدعوة اللي لسه شغّالة بيرجع ٤٠٩
  //    (ما بنكسرش رابطًا في الطريق)، واللي انضمّ صاحبها ٤٠٩ كمان (حسابه قائم).
  EP.reissueInvite = function (inv, onLink, after) {
    post("/invites/" + encodeURIComponent(inv.id) + "/reissue", {})
      .then(function (r) {
        note("اتعمل رابط جديد لـ" + (inv.email || inv.name || "بلا اسم") + " — القديم بطل");
        EP.reload("invites", after);
        if (onLink) onLink(r || {});
      })
      .catch(function (e) {
        quietToast(e && e.status === 404
          ? "مسار إعادة الإصدار لسه ما اتنشرش"
          : ((e && e.message) || "تعذّرت إعادة إصدار الدعوة"));
        if (after) after();
      });
  };
  ...
  // F-001: الرقم وحده هو اللي بيتبعت من هنا — الدعوة بتترابط برقم واتساب، والمنصة بترفض بلا رقم.
  // الشاشة بتسأل عنه في **كل** تحويل (مؤكَّدًا أو معدَّلًا) قبل النداء، بدل ٤٢٢ في وشّ المؤسس.
  // والمنصة بتقدّم الرقم اللي جاي من هنا على هاتف الصفّ — اللي كتبه المؤسس دلوقتي هو الحكم.
  EP.inviteFromWaitlist = function (row, onLink, after) {
    post("/waitlist/" + encodeURIComponent(row.id) + "/invite", { phone: (row && row.phone) || "" })
      .then(function (r) {
