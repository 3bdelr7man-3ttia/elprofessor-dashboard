...
const investmentStatusLabels = { accrued: "مستحق", paid: "مدفوع", pending: "قيد التسوية", active: "نشط", completed: "مكتمل", loss: "خسارة" };
const opportunityStatusLabels = { open: "مفتوح للجميع", hot: "طلب عالي", invite_only: "بطلب فقط", funded: "مكتمل التمويل", closed: "مغلق" };
const opportunityStatusColors = { open: "#059669", hot: "#D4AF37", invite_only: "#98A2B3", funded: "#0f4c81", closed: "#98A2B3" };
const investorLevelLabels = { bronze: "مستثمر برونزي", silver: "مستثمر فضي", gold: "مستثمر ذهبي", platinum: "مستثمر بلاتيني" };
...
function StatusPill({ text, color, subtle = false }) {
  return (
    <span style={{
      display: "inline-flex",
      alignItems: "center",
...
function ProgressBar({ value = 0, color = `linear-gradient(90deg, ${BRAND.gold}, ${BRAND.amber})`, track = "#e5e7eb" }) {
  const width = Math.max(0, Math.min(100, Number(value || 0)));
  return (
    <div style={{ height: 10, borderRadius: 999, background: track, overflow: "hidden" }}>
      <div style={{ width: `${width}%`, height: "100%", background: color, borderRadius: 999, transition: "width 180ms ease" }} />
    </div>
  );
}

function WalletHero({ wallet, onWithdraw, compact = false }) {
  const stats = [
    { label: "إجمالي المُستثمر", value: fmtUSD(wallet?.total_invested || 0), sub: egpLabel(wallet?.total_invested_egp || 0) },
    { label: "أرباح محققة", value: fmtUSD(wallet?.total_returns || 0), sub: egpLabel(wallet?.total_returns_egp || 0) },
    { label: "متوسط العائد", value: `${Number(wallet?.avg_roi || 0).toFixed(1)}%`, sub: wallet?.pending_withdrawals ? `معلّق للسحب: ${fmtUSD(wallet.pending_withdrawals)}` : "أداء استثماراتك" },
  ];
  return (
    <div style={{ borderRadius: 18, padding: compact ? 22 : 28, color: "#fff", background: "linear-gradient(135deg, #1a2b4a 0%, #0f4c81 60%, #1565a3 100%)", boxShadow: "0 20px 50px rgba(15,76,129,0.18)", marginBottom: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "start", flexWrap: "wrap", marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.78)", marginBottom: 8 }}>محفظة المستثمر</div>
          <div style={{ fontSize: compact ? 28 : 32, fontWeight: 900, lineHeight: 1.1 }}>{fmtUSD(wallet?.balance || 0)}</div>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.76)", marginTop: 6 }}>{egpLabel(wallet?.balance_egp || 0)}</div>
        </div>
        <StatusPill text={investorLevelLabels[wallet?.level] || "مستثمر"} color={BRAND.gold} />
      </div>
      <div className="wallet-stats" style={{ display: "grid", gridTemplateColumns: compact ? "repeat(auto-fit, minmax(160px, 1fr))" : "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
        {stats.map((stat) => (
          <div key={stat.label} style={{ borderRadius: 14, padding: "14px 16px", background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,255,255,0.12)" }}>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.76)", marginBottom: 6 }}>{stat.label}</div>
            <div style={{ fontSize: 18, fontWeight: 900 }}>{stat.value}</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", marginTop: 4 }}>{stat.sub}</div>
          </div>
        ))}
      </div>
      {onWithdraw && (
        <div className="withdraw-bar" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginTop: 18, paddingTop: 18, borderTop: "1px solid rgba(255,255,255,0.12)" }}>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.88)" }}>رصيد قابل للسحب: <strong>{fmtUSD(wallet?.balance || 0)}</strong></div>
          <button onClick={onWithdraw} style={{ minHeight: 44, borderRadius: 12, border: "1px solid rgba(212,175,55,0.8)", color: "#fff", background: "transparent", padding: "10px 18px", fontWeight: 800, cursor: "pointer" }}>طلب سحب</button>
        </div>
      )}
    </div>
  );
...
function AchievementStrip({ badges = [], level = "bronze" }) {
  return (
    <PageSection title="إنجازاتك الاستثمارية" subtitle={investorLevelLabels[level] || "مستثمر"}>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        {badges.map((badge) => (
          <div key={badge.badge_type} style={{
            width: 110,
            borderRadius: 16,
            padding: "16px 10px",
            textAlign: "center",
            ...
            <div style={{
              ...
              background: badge.earned ? `${BRAND.gold}1a` : "#fff",
              color: badge.earned ? BRAND.gold : "#98A2B3",
              border: badge.earned ? `1px solid ${BRAND.gold}40` : "1px dashed #cbd5e1",
              fontWeight: 900,
            }}>{badge.earned ? "✓" : "?"}</div>
            <div style={{ fontSize: 12, fontWeight: 800, color: BRAND.ink }}>{badge.label}</div>
          </div>
        ))}
      </div>
    </PageSection>
...
function MarketingPage() {
  ...
  const [marketTab, setMarketTab] = useState("opportunities");
  const [manageTab, setManageTab] = useState("opportunities");
  const [campaigns, setCampaigns] = useState([]);
  const [courses, setCourses] = useState([]);
  const [opportunities, setOpportunities] = useState([]);
  const [activeInvestments, setActiveInvestments] = useState([]);
  const [historyInvestments, setHistoryInvestments] = useState([]);
  ...
  const [opportunityForm, setOpportunityForm] = useState({ status: "open", trainer_pct: 35, platform_pct: 35, investors_pct: 30, affiliate_pct: 0, min_investment: 100 });
  ...
  const load = useCallback(() => {
    ...
    api.get("/investment-opportunities").then((data) => setOpportunities(safeArray(data)));
    api.get("/investments/active").then((data) => setActiveInvestments(safeArray(data)));
    api.get("/investments/history").then((data) => setHistoryInvestments(safeArray(data)));
    api.get("/ai/investment-recommendation").then((data) => setRecommendation(safeObject(data)));
    if (effectiveRole === "investor") {
      api.get("/investor/wallet").then((data) => setWallet(data?.error ? null : safeObject(data)));
      api.get("/investor/badges").then((data) => setBadges(safeArray(data)));
    }
    if (isAdmin) {
      api.get("/admin/wallets").then((data) => setWallets(safeArray(data)));
      api.get("/admin/withdrawals").then((data) => setWithdrawals(safeArray(data)));
    }
  ...
  const investmentTotals = {
    injected: [...activeInvestments, ...historyInvestments].reduce((sum, item) => sum + (item.amount_usd || 0), 0),
    distributed: [...activeInvestments, ...historyInvestments].reduce((sum, item) => sum + ((item.actual_return || 0) || ((item.due_egp || 0) / 50)), 0),
    active: new Set(activeInvestments.map(item => item.investor_name)).size,
    avgRoi: [...activeInvestments, ...historyInvestments].length ? [...activeInvestments, ...historyInvestments].reduce((sum, item) => sum + (item.roi_percent || 0), 0) / [...activeInvestments, ...historyInvestments].length : 0,
  };
  ...
  const saveOpportunity = async () => {
    if (!opportunityForm.course_id) return alert("اختر الدورة أولًا.");
    const totalPercent = Number(opportunityForm.trainer_pct || 0) + Number(opportunityForm.platform_pct || 0) + Number(opportunityForm.investors_pct || 0) + Number(opportunityForm.affiliate_pct || 0);
    if (Math.abs(totalPercent - 100) > 0.01) return alert("مجموع النسب يجب أن يساوي 100%");
    if (opportunityForm.id) await api.put(`/investment-opportunities/${opportunityForm.id}`, opportunityForm);
    else await api.post("/investment-opportunities", opportunityForm);
    setOpportunityModal(false);
    setOpportunityForm({ status: "open", trainer_pct: 35, platform_pct: 35, investors_pct: 30, affiliate_pct: 0, min_investment: 100 });
    load();
  };
  ...
  const submitInvestment = async () => {
    ...
    if (!amount) return alert("اكتب مبلغ الاستثمار.");
    if (amount < Number(selectedOpportunity.min_investment || 0)) return alert(`الحد الأدنى للاستثمار هو ${fmtUSD(selectedOpportunity.min_investment)}.`);
    await api.post("/investments", {
      opportunity_id: selectedOpportunity.id,
      course_id: selectedOpportunity.course_id,
      investor_name: user?.linked_to_name || user?.name,
      investor_user_id: user?.id,
      amount_usd: amount,
      amount_egp: amount * 50,
      profit_percent: selectedOpportunity.investors_pct,
      status: "active",
      notes: `استثمار من الماركت بليس في ${selectedOpportunity.course_title}`,
    });
  ...
  const submitWithdrawal = async () => {
    ...
    if (!amount) return alert("اكتب مبلغ السحب.");
    const r = await api.post("/investor/withdraw", { amount, currency: "USD" });
    if (r.error) return alert(r.error);
    alert(r.message || "تم إرسال الطلب");
  ...
  const openInvestModal = (opportunity) => {
    if (opportunity.status === "invite_only") {
      alert("هذه الدورة بطلَب فقط. سنضيف لاحقًا تدفق تقديم الطلب.");
      return;
    }
    setSelectedOpportunity(opportunity);
    setInvestAmount(opportunity.min_investment || "");
    setInvestModal(true);
  };

  const investAmountNumber = Number(investAmount || 0);
  const selectedTarget = Number(selectedOpportunity?.target_amount || 0);
  const selectedExpectedValue = Number(selectedOpportunity?.expected_return_value || 0);
  const contributionShare = selectedTarget ? Math.min(100, (investAmountNumber / selectedTarget) * 100) : 0;
  const expectedPayout = selectedTarget ? (investAmountNumber / selectedTarget) * selectedExpectedValue : 0;
  const expectedNetProfit = expectedPayout - investAmountNumber;
  ...
  const renderCampaigns = () => (
    <div>
      ...
      <PageSection title="الحملات الحالية" subtitle="الحملات الإعلانية تبقى مستقلة، والاستثمار أصبح مسارًا منفصلًا في الماركت بليس.">
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead><tr style={{ background: "#f8fafc" }}>
  ...
  const renderOpportunityCard = (opportunity) => {
    const isInvite = opportunity.status === "invite_only";
    const progressColor = opportunity.status === "hot" ? `linear-gradient(90deg, ${BRAND.gold}, ${BRAND.amber})` : "linear-gradient(90deg, #059669, #10b981)";
    return (
      <div key={opportunity.id} style={{
        ...
        padding: 18,
        boxShadow: "0 10px 30px rgba(16,27,47,0.05)",
        display: "grid",
        gap: 14,
        opacity: isInvite ? 0.65 : 1,
        transition: "transform 0.18s ease, box-shadow 0.18s ease",
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 900, color: BRAND.ink }}>{opportunity.course_title}</div>
            <div style={{ marginTop: 6, fontSize: 11, color: "#667085" }}>{opportunity.programs_count || 1} برنامج تدريبي • {opportunity.expected_start ? `${opportunity.expected_start} - ${opportunity.expected_end || "مفتوح"}` : "تاريخ سيحدد لاحقًا"}</div>
          </div>
        ...
        <div className="card-meta" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12 }}>
          <div><div style={{ fontSize: 11, color: "#667085" }}>الحد الأدنى للاستثمار</div><div style={{ fontSize: 18, fontWeight: 900, color: BRAND.navy }}>{fmtUSD(opportunity.min_investment)}</div></div>
          <div><div style={{ fontSize: 11, color: "#667085" }}>العائد المتوقع</div><div style={{ fontSize: 18, fontWeight: 900, color: "#059669" }}>{opportunity.status === "invite_only" ? "—" : `${Number(opportunity.expected_roi || 0).toFixed(0)}%`}</div></div>
          <div><div style={{ fontSize: 11, color: "#667085" }}>بداية متوقعة</div><div style={{ fontSize: 15, fontWeight: 800, color: BRAND.ink }}>{opportunity.expected_start || "قريبًا"}</div></div>
        </div>
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#667085", marginBottom: 8 }}>
            <span>التمويل المطلوب: {fmtUSD(opportunity.target_amount)}</span>
            <span>{Number(opportunity.funding_percent || 0).toFixed(0)}%</span>
          </div>
          <ProgressBar value={opportunity.funding_percent} color={progressColor} />
        </div>
        <div style={{ fontSize: 12, color: "#667085" }}>نسبتك من الأرباح تُحسب تلقائيًا بناءً على مساهمتك من إجمالي التمويل.</div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <Btn color={isInvite ? BRAND.navy : BRAND.gold} onClick={() => openInvestModal(opportunity)} style={{ minHeight: 44, flex: 1, color: isInvite ? "#fff" : BRAND.navy }}>{isInvite ? "تقديم طلب انضمام" : "استثمر الآن"}</Btn>
          <Btn variant="secondary" color={BRAND.navy} onClick={() => alert(opportunity.notes || "سيتم عرض صفحة التفاصيل الكاملة لاحقًا.")} style={{ minHeight: 44, flex: 1 }}>التفاصيل</Btn>
        </div>
      </div>
    );
  };
  ...
  const renderMarketplace = () => (
    <div>
      {wallet && <WalletHero wallet={wallet} onWithdraw={() => setWithdrawModal(true)} />}
      {!wallet && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, marginBottom: 20 }}>
          <KPICard icon="🏦" label="إجمالي الاستثمارات المضخوخة" value={fmtUSD(investmentTotals.injected)} color={BRAND.navy} />
          <KPICard icon="💵" label="إجمالي العوائد الموزعة" value={fmtUSD(investmentTotals.distributed)} color="#2d8659" />
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        {[
          ...
          <button key={id} onClick={() => setMarketTab(id)} style={{ minHeight: 44, padding: "10px 16px", borderRadius: 12, border: marketTab === id ? "none" : "1px solid #d0d5dd", background: marketTab === id ? BRAND.navy : "#fff", color: marketTab === id ? "#fff" : "#475467", fontWeight: 800, cursor: "pointer" }}>{label}</button>
        ))}
      </div>

      {marketTab === "opportunities" && (
        <div style={{ display: "grid", gap: 20 }}>
          {recommendation?.title && (
            <div style={{ borderRadius: 16, background: "#FDF8E8", border: "1px solid #f4e2a8", padding: 16, display: "flex", gap: 14, alignItems: "center" }}>
              <div style={{ width: 42, height: 42, borderRadius: "50%", background: `${BRAND.gold}22`, color: BRAND.gold, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900 }}>AI</div>
              <div style={{ fontSize: 13, color: "#5b6475", lineHeight: 1.8 }}>
                <strong style={{ color: BRAND.gold }}>{recommendation.title}</strong> — {recommendation.message}
              </div>
            </div>
          )}

          <PageSection title="كيف يعمل نموذج الاستثمار في البروفيسور؟" subtitle="شاهد الفيديو التوضيحي لفهم آلية الاستثمار وتوزيع الأرباح">
            <div style={{ borderRadius: 18, background: "#1a2b4a", minHeight: 220, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
          ...
          {wallet && <AchievementStrip badges={badges} level={wallet.level} />}
        </div>
      )}
            ...
            <div key={item.id} style={{ background: "#fff", borderRadius: 16, border: "1px solid #e6ecf2", padding: 18, display: "grid", gap: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}>
                <div>
                  ...
                  <div style={{ marginTop: 6, fontSize: 12, color: "#667085" }}>{item.created_at?.slice(0, 10) || "—"} • {fmtUSD(item.amount_usd || 0)}</div>
                </div>
              ...
              <div className="card-meta" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 12 }}>
                <div><div style={{ fontSize: 11, color: "#667085" }}>عدد الطلاب</div><div style={{ fontWeight: 900 }}>{item.opportunity?.students_count || item.opportunity?.programs_count || 0}</div></div>
                <div><div style={{ fontSize: 11, color: "#667085" }}>الأرباح المتوقعة</div><div style={{ fontWeight: 900, color: "#059669" }}>{fmtUSD((item.due_egp || 0) / 50)}</div></div>
                <div><div style={{ fontSize: 11, color: "#667085" }}>الوقت المتبقي</div><div style={{ fontWeight: 900 }}>{item.opportunity?.expected_end || "قريبًا"}</div></div>
              </div>
              <ProgressBar value={item.opportunity?.funding_percent || item.share_pct || 0} color="linear-gradient(90deg, #059669, #10b981)" />
            </div>
          )) : <EmptyState title="لا توجد استثمارات نشطة" text="عند دخولك في دورة ستظهر هنا متابعة الأداء والطلاب والعائد المتوقع." />}
        </div>
      )}

      {marketTab === "history" && (
        <PageSection title="السجل" subtitle="كل العمليات السابقة، مع تمييز الخسائر باللون الأحمر">
          {historyInvestments.length ? historyInvestments.map((item) => (
            <div key={item.id} style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", padding: "14px 0", borderTop: "1px solid #f3f4f6" }}>
              <div>
                ...
                <div style={{ marginTop: 6, fontSize: 12, color: "#667085" }}>{item.created_at?.slice(0, 10) || "—"} • استثمار {fmtUSD(item.amount_usd || 0)}</div>
                {item.status === "loss" && <div style={{ marginTop: 6, fontSize: 12, color: "#DC2626" }}>نسبة أعلى في الدورة القادمة</div>}
              </div>
              <div style={{ textAlign: "left" }}>
                <div style={{ fontWeight: 900, color: item.status === "loss" ? "#DC2626" : "#059669" }}>{fmtUSD(item.actual_return || ((item.due_egp || 0) / 50))}</div>
                <div style={{ marginTop: 6, fontSize: 12, color: "#667085" }}>{item.roi_percent?.toFixed?.(1) ?? item.roi_percent}%</div>
              </div>
            </div>
          )) : <EmptyState title="لا توجد عمليات سابقة" text="بعد اكتمال أول استثمار ستبدأ السجلات في الظهور هنا." />}
        </PageSection>
      )}
    </div>
  );
  ...
  const renderManage = () => (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        {[
          ["opportunities", "إضافة فرص الاستثمار"],
          ["wallets", "إدارة المحافظ"],
          ["withdrawals", "طلبات السحب"],
        ].map(([id, label]) => (
          <button key={id} onClick={() => setManageTab(id)} style={{ minHeight: 44, padding: "10px 16px", borderRadius: 12, border: manageTab === id ? "none" : "1px solid #d0d5dd", background: manageTab === id ? BRAND.navy : "#fff", color: manageTab === id ? "#fff" : "#475467", fontWeight: 800, cursor: "pointer" }}>{label}</button>
        ))}
      </div>

      {manageTab === "opportunities" && (
        <PageSection title="إدارة فرص الاستثمار" subtitle="حدّد المبلغ المطلوب والنسب وحالة الطرح لكل دورة." action={<Btn color={BRAND.gold} style={{ color: BRAND.navy }} onClick={() => { setOpportunityForm({ status: "open", trainer_pct: 35, platform_pct: 35, investors_pct: 30, affiliate_pct: 0, min_investment: 100 }); setOpportunityModal(true); }}>+ فرصة جديدة</Btn>}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#f8fafc" }}>
              {["الدورة", "التمويل المطلوب", "الممول حاليًا", "العائد المتوقع", "النسب", "الحالة", ""].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "right", color: "#667085", borderBottom: "1px solid #e5e7eb" }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {opportunities.map(item => (
                <tr key={item.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  <td style={{ padding: "12px 14px", fontWeight: 800 }}>{item.course_title}</td>
                  <td style={{ padding: "12px 14px" }}>{fmtUSD(item.target_amount)}</td>
                  <td style={{ padding: "12px 14px" }}>{fmtUSD(item.current_funded)}</td>
                  <td style={{ padding: "12px 14px", color: "#059669", fontWeight: 800 }}>{item.expected_roi}%</td>
                  <td style={{ padding: "12px 14px", fontSize: 12, color: "#667085" }}>مدرب {item.trainer_pct}% • منصة {item.platform_pct}% • مستثمرون {item.investors_pct}% • أفلييت {item.affiliate_pct}%</td>
                  <td style={{ padding: "12px 14px" }}><StatusPill text={opportunityStatusLabels[item.status] || item.status} color={opportunityStatusColors[item.status] || BRAND.navy} subtle /></td>
                  <td style={{ padding: "12px 14px" }}>
                    <button onClick={() => { setOpportunityForm(item); setOpportunityModal(true); }} style={{ background: "none", border: "none", cursor: "pointer" }}>✏️</button>
                    <button onClick={async () => { if (confirm("حذف الفرصة؟")) { await api.del(`/investment-opportunities/${item.id}`); load(); } }} style={{ background: "none", border: "none", cursor: "pointer" }}>🗑</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {manageTab === "wallets" && (
        <PageSection title="إدارة المحافظ">
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#f8fafc" }}>
              {["المستثمر", "إجمالي مستثمر", "أرباح محققة", "رصيد متاح", "طلبات سحب معلقة", "المستوى"].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "right", color: "#667085", borderBottom: "1px solid #e5e7eb" }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {wallets.map(item => (
                <tr key={item.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  <td style={{ padding: "12px 14px", fontWeight: 800 }}>{item.investor_name}</td>
                  <td style={{ padding: "12px 14px" }}>{fmtUSD(item.total_invested)}</td>
                  <td style={{ padding: "12px 14px", color: "#059669", fontWeight: 800 }}>{fmtUSD(item.total_returns)}</td>
                  <td style={{ padding: "12px 14px" }}>{fmtUSD(item.balance)}</td>
                  <td style={{ padding: "12px 14px" }}>{item.pending_requests || 0}</td>
                  <td style={{ padding: "12px 14px" }}>{investorLevelLabels[item.level] || item.level}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {manageTab === "withdrawals" && (
        <PageSection title="طلبات السحب">
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#f8fafc" }}>
              {["المستثمر", "المبلغ", "تاريخ الطلب", "الحالة", ""].map(h => <th key={h} style={{ padding: "12px 14px", textAlign: "right", color: "#667085", borderBottom: "1px solid #e5e7eb" }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {withdrawals.map(item => (
                <tr key={item.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  <td style={{ padding: "12px 14px", fontWeight: 800 }}>{item.investor_name}</td>
                  <td style={{ padding: "12px 14px" }}>{fmtUSD(item.amount)}<div style={{ fontSize: 12, color: "#94a3b8" }}>{egpLabel(item.amount_egp)}</div></td>
                  <td style={{ padding: "12px 14px" }}>{item.requested_at?.slice(0, 10) || "—"}</td>
                  <td style={{ padding: "12px 14px" }}><StatusPill text={item.status} color={item.status === "pending" ? BRAND.gold : item.status === "approved" ? "#059669" : "#98A2B3"} subtle /></td>
                  <td style={{ padding: "12px 14px" }}>
                    {item.status === "pending" && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <Btn color="#059669" onClick={() => updateWithdrawal(item.id, "approved")} style={{ minHeight: 40, padding: "8px 14px" }}>موافقة</Btn>
                        <Btn variant="secondary" color="#DC2626" onClick={() => updateWithdrawal(item.id, "rejected")} style={{ minHeight: 40, padding: "8px 14px" }}>رفض</Btn>
                      </div>
                    )}
                  </td>
                </tr>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ marginTop: 6, fontSize: 13, color: "#667085" }}>الحملات الإعلانية الآن منفصلة عن مسار الاستثمار، والذي صار ماركت بليس مستقلًا للمستثمرين والمحافظ.</div>
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          {isAdmin && <Btn onClick={() => { setOpportunityForm({ status: "open", trainer_pct: 35, platform_pct: 35, investors_pct: 30, affiliate_pct: 0, min_investment: 100 }); setOpportunityModal(true); }} color={BRAND.navy}>+ فرصة استثمار</Btn>}
        </div>
      </div>

      <TabBar tabs={[{ id: "campaigns", label: "الحملات" }, { id: "marketplace", label: "ماركت الاستثمار" }, ...(isAdmin ? [{ id: "manage", label: "إدارة الاستثمارات" }] : [])]} active={tab} onChange={setTab} />

      {tab === "campaigns" && renderCampaigns()}
      {tab === "marketplace" && renderMarketplace()}
      {tab === "manage" && isAdmin && renderManage()}
      ...
      <Modal open={opportunityModal} onClose={() => setOpportunityModal(false)} title={opportunityForm.id ? "تعديل فرصة استثمار" : "فرصة استثمار جديدة"} maxWidth={760}>
        <Select label="الدورة" value={opportunityForm.course_id || ""} onChange={e => setOpportunityForm({ ...opportunityForm, course_id: +e.target.value })} options={[{ value: "", label: "اختر الدورة" }, ...courses.map(course => ({ value: course.id, label: course.title }))]} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <Input label="المبلغ المطلوب ($)" type="number" value={opportunityForm.target_amount || 0} onChange={e => setOpportunityForm({ ...opportunityForm, target_amount: +e.target.value })} />
          <Input label="الحد الأدنى للاستثمار ($)" type="number" value={opportunityForm.min_investment || 0} onChange={e => setOpportunityForm({ ...opportunityForm, min_investment: +e.target.value })} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <Input label="العائد المتوقع %" type="number" value={opportunityForm.expected_roi || 0} onChange={e => setOpportunityForm({ ...opportunityForm, expected_roi: +e.target.value })} />
          <Input label="البداية المتوقعة" type="date" value={opportunityForm.expected_start || ""} onChange={e => setOpportunityForm({ ...opportunityForm, expected_start: e.target.value })} />
          <Input label="النهاية المتوقعة" type="date" value={opportunityForm.expected_end || ""} onChange={e => setOpportunityForm({ ...opportunityForm, expected_end: e.target.value })} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
          {[
            ["المدرب %", "trainer_pct"],
            ["المنصة %", "platform_pct"],
            ["المستثمرون %", "investors_pct"],
            ["الأفلييت %", "affiliate_pct"],
          ].map(([label, key]) => <Input key={key} label={label} type="number" value={opportunityForm[key] || 0} onChange={e => setOpportunityForm({ ...opportunityForm, [key]: +e.target.value })} />)}
        </div>
        <Select label="حالة الطرح" value={opportunityForm.status || "open"} onChange={e => setOpportunityForm({ ...opportunityForm, status: e.target.value })} options={Object.entries(opportunityStatusLabels).map(([value, label]) => ({ value, label }))} />
        <Input label="ملاحظات" value={opportunityForm.notes || ""} onChange={e => setOpportunityForm({ ...opportunityForm, notes: e.target.value })} />
        <Btn onClick={saveOpportunity} color={BRAND.navy} style={{ width: "100%" }}>حفظ فرصة الاستثمار</Btn>
      </Modal>

      <Modal open={investModal} onClose={() => setInvestModal(false)} title={selectedOpportunity ? `استثمر في ${selectedOpportunity.course_title}` : "استثمر الآن"} maxWidth={680}>
        {selectedOpportunity && (
          <div style={{ display: "grid", gap: 16 }}>
            <Input label="المبلغ المراد استثماره ($)" type="number" value={investAmount} onChange={e => setInvestAmount(e.target.value)} />
            <div style={{ borderRadius: 14, background: "#f8fafc", border: "1px solid #e5e7eb", padding: 14, fontSize: 13, color: "#667085", lineHeight: 1.8 }}>
              النسب محددة مسبقًا من المنصة لهذه الدورة: المدرب {selectedOpportunity.trainer_pct}%، المنصة {selectedOpportunity.platform_pct}%، المستثمرون {selectedOpportunity.investors_pct}%، الأفلييت {selectedOpportunity.affiliate_pct}%.
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>مساهمتك من إجمالي التمويل</div><div style={{ fontSize: 18, fontWeight: 900, color: BRAND.navy }}>{contributionShare.toFixed(1)}%</div></div>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>حصتك من نسبة المستثمرين</div><div style={{ fontSize: 18, fontWeight: 900, color: BRAND.navy }}>{((selectedOpportunity.investors_pct || 0) * contributionShare / 100).toFixed(1)}%</div></div>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>العائد المتوقع</div><div style={{ fontSize: 18, fontWeight: 900, color: "#059669" }}>{fmtUSD(expectedPayout)}</div></div>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>صافي الربح المتوقع</div><div style={{ fontSize: 18, fontWeight: 900, color: expectedNetProfit >= 0 ? "#059669" : "#DC2626" }}>{fmtUSD(expectedNetProfit)}</div></div>
            </div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <Btn color={BRAND.gold} onClick={submitInvestment} style={{ color: BRAND.navy, flex: 1, minHeight: 44 }}>تأكيد الاستثمار</Btn>
              <Btn variant="secondary" color={BRAND.navy} onClick={() => setInvestModal(false)} style={{ flex: 1, minHeight: 44 }}>إلغاء</Btn>
            </div>
          </div>
        )}
      </Modal>

      <Modal open={withdrawModal} onClose={() => setWithdrawModal(false)} title="طلب سحب" maxWidth={520}>
        <Input label="المبلغ المطلوب سحبه ($)" type="number" value={withdrawAmount} onChange={e => setWithdrawAmount(e.target.value)} />
        <div style={{ fontSize: 12, color: "#667085", marginBottom: 12, lineHeight: 1.8 }}>الحد الأدنى للسحب 50 دولار، ويتم تنفيذ الطلب خلال 7 أيام عمل من تاريخ الموافقة.</div>
        <Btn color={BRAND.navy} onClick={submitWithdrawal} style={{ width: "100%", minHeight: 44 }}>تأكيد الطلب</Btn>
      </Modal>
    </div>
  );
}
...
function InvestorInvestmentsPage({ initialTab = "opportunities" }) {
  ...
  const openInvestModal = (opportunity) => {
    if (opportunity.status === "invite_only") {
      alert("هذه الدورة بطلب فقط. سنضيف تدفق التقديم لاحقًا.");
      return;
    }
    setSelectedOpportunity(opportunity);
    setInvestAmount(opportunity.min_investment || "");
  ...
  const submitInvestment = async () => {
    ...
    await api.post("/investments", {
      opportunity_id: selectedOpportunity.id,
      course_id: selectedOpportunity.course_id,
      amount_usd: investAmountNumber,
      amount_egp: investAmountNumber * 50,
      profit_percent: selectedOpportunity.investors_pct,
      status: "active",
    });
    setInvestModal(false);
  ...
  const submitWithdrawal = async () => {
    const r = await api.post("/investor/withdraw", { amount: Number(withdrawAmount || 0), currency: "USD" });
    if (r.error) return alert(r.error);
    alert(r.message || "تم إرسال الطلب");
    setWithdrawModal(false);
    setWithdrawAmount("");
  ...
  return (
    <div>
      <h1 style={{ fontSize: 24, fontWeight: 900, color: BRAND.navy, marginBottom: 20 }}>{initialTab === "opportunities" ? "ماركت الاستثمار" : "استثماراتي"}</h1>
      {wallet && <WalletHero wallet={wallet} onWithdraw={() => setWithdrawModal(true)} />}
      ...
      {tab === "opportunities" && (
        <div style={{ display: "grid", gap: 20 }}>
          ...
          <PageSection title="كيف يعمل نموذج الاستثمار في البروفيسور؟" subtitle="فيديو توضيحي سيُربط لاحقًا بمحتوى فعلي">
            <div style={{ borderRadius: 18, background: "#1a2b4a", minHeight: 220, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ width: 72, height: 72, borderRadius: "50%", background: BRAND.gold, color: BRAND.navy, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 900 }}>▶</div>
            </div>
          </PageSection>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 18 }}>
            {opportunities.map((opportunity) => (
              <div key={opportunity.id} style={{ background: "#fff", borderRadius: 16, border: "1px solid #e6ecf2", padding: 18, display: "grid", gap: 14, opacity: opportunity.status === "invite_only" ? 0.65 : 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 900, color: BRAND.ink }}>{opportunity.course_title}</div>
                    <div style={{ marginTop: 6, fontSize: 11, color: "#667085" }}>{opportunity.programs_count || 1} برنامج • {opportunity.expected_start || "قريبًا"}</div>
                  </div>
                  <StatusPill text={opportunityStatusLabels[opportunity.status] || opportunity.status} color={opportunityStatusColors[opportunity.status] || BRAND.navy} subtle />
                </div>
                <div className="card-meta" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12 }}>
                  <div><div style={{ fontSize: 11, color: "#667085" }}>الحد الأدنى</div><div style={{ fontWeight: 900 }}>{fmtUSD(opportunity.min_investment)}</div></div>
                  ...
                  <div><div style={{ fontSize: 11, color: "#667085" }}>التمويل المطلوب</div><div style={{ fontWeight: 900 }}>{fmtUSD(opportunity.target_amount)}</div></div>
                </div>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <Btn color={opportunity.status === "invite_only" ? BRAND.navy : BRAND.gold} onClick={() => openInvestModal(opportunity)} style={{ minHeight: 44, flex: 1, color: opportunity.status === "invite_only" ? "#fff" : BRAND.navy }}>{opportunity.status === "invite_only" ? "تقديم طلب انضمام" : "استثمر الآن"}</Btn>
                  <Btn variant="secondary" color={BRAND.navy} onClick={() => alert(opportunity.notes || "ستتوفر صفحة التفاصيل الموسعة لاحقًا.")} style={{ minHeight: 44, flex: 1 }}>التفاصيل</Btn>
                </div>
              </div>
            ))}
          </div>
          ...
          {activeInvestments.length ? activeInvestments.map((item) => (
            <div key={item.id} style={{ background: "#fff", borderRadius: 16, border: "1px solid #e6ecf2", padding: 18, display: "grid", gap: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}>
                <div>
                  <div style={{ fontWeight: 900, color: BRAND.ink }}>{item.course_title}</div>
                  <div style={{ marginTop: 6, fontSize: 12, color: "#667085" }}>{fmtUSD(item.amount_usd || 0)} • {item.created_at?.slice(0, 10) || "—"}</div>
                </div>
                <StatusPill text="جارية" color="#059669" subtle />
              </div>
              <div className="card-meta" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 12 }}>
                <div><div style={{ fontSize: 11, color: "#667085" }}>عدد الطلاب</div><div style={{ fontWeight: 900 }}>{item.opportunity?.students_count || 0}</div></div>
                <div><div style={{ fontSize: 11, color: "#667085" }}>الأرباح المتوقعة</div><div style={{ fontWeight: 900, color: "#059669" }}>{fmtUSD((item.due_egp || 0) / 50)}</div></div>
                <div><div style={{ fontSize: 11, color: "#667085" }}>الوقت المتبقي</div><div style={{ fontWeight: 900 }}>{item.opportunity?.expected_end || "قريبًا"}</div></div>
              </div>
              <ProgressBar value={item.opportunity?.funding_percent || item.share_pct || 0} color="linear-gradient(90deg, #059669, #10b981)" />
            </div>
          )) : <EmptyState title="لا توجد استثمارات نشطة" text="بمجرد دخولك في أي فرصة ستظهر هنا متابعة الدورة والعائد المتوقع." />}
        </div>
      )}

      {tab === "history" && (
        <PageSection title="السجل">
          {historyInvestments.length ? historyInvestments.map((item) => (
            <div key={item.id} style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", padding: "14px 0", borderTop: "1px solid #f3f4f6" }}>
              <div>
                <div style={{ fontWeight: 800, color: BRAND.ink }}>{item.course_title}</div>
      ...
      <Modal open={investModal} onClose={() => setInvestModal(false)} title={selectedOpportunity ? `استثمر في ${selectedOpportunity.course_title}` : "استثمر الآن"} maxWidth={680}>
        {selectedOpportunity && (
          <div style={{ display: "grid", gap: 16 }}>
            <Input label="المبلغ المراد استثماره ($)" type="number" value={investAmount} onChange={e => setInvestAmount(e.target.value)} />
            <div style={{ borderRadius: 14, background: "#f8fafc", border: "1px solid #e5e7eb", padding: 14, fontSize: 13, color: "#667085", lineHeight: 1.8 }}>
              النسب محددة مسبقًا من المنصة: مدرب {selectedOpportunity.trainer_pct}%، منصة {selectedOpportunity.platform_pct}%، مستثمرون {selectedOpportunity.investors_pct}%، أفلييت {selectedOpportunity.affiliate_pct}%.
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>مساهمتك من التمويل</div><div style={{ fontSize: 18, fontWeight: 900, color: BRAND.navy }}>{contributionShare.toFixed(1)}%</div></div>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>حصتك من نسبة المستثمرين</div><div style={{ fontSize: 18, fontWeight: 900, color: BRAND.navy }}>{((selectedOpportunity.investors_pct || 0) * contributionShare / 100).toFixed(1)}%</div></div>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>العائد المتوقع</div><div style={{ fontSize: 18, fontWeight: 900, color: "#059669" }}>{fmtUSD(expectedPayout)}</div></div>
              <div style={{ borderRadius: 12, border: "1px solid #edf2f7", padding: 14 }}><div style={{ fontSize: 11, color: "#667085" }}>صافي الربح المتوقع</div><div style={{ fontSize: 18, fontWeight: 900, color: expectedNetProfit >= 0 ? "#059669" : "#DC2626" }}>{fmtUSD(expectedNetProfit)}</div></div>
            </div>
      ...
      <Modal open={withdrawModal} onClose={() => setWithdrawModal(false)} title="طلب سحب" maxWidth={520}>
        <Input label="المبلغ المطلوب سحبه ($)" type="number" value={withdrawAmount} onChange={e => setWithdrawAmount(e.target.value)} />
        <div style={{ fontSize: 12, color: "#667085", marginBottom: 12, lineHeight: 1.8 }}>الحد الأدنى 50 دولار، والمعالجة خلال 7 أيام عمل.</div>
        <Btn color={BRAND.navy} onClick={submitWithdrawal} style={{ width: "100%", minHeight: 44 }}>إرسال الطلب</Btn>
      </Modal>
    </div>
  );
}
